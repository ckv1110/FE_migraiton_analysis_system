...
def main():
    ...
    if prev_itr == 0:
        ...
        # Change to new_itr in 1st var for relative, prev_itr for abs
        new_feb_file = name + '_' + str(new_itr).zfill(2) + '_' + str(new_itr + 8).zfill(2) + '_v2_redo.feb'  # to make 64_72 file
    else:
        #Change to new_itr in 1st var for relative, prev_itr for abs
        new_feb_file = name + '_' + str(new_itr).zfill(2) + '_' + str(new_itr + 9).zfill(2) + '_v2_redo.feb'
    ...
    def overwrite_log_line(edit, type):
        ...
        if type == 'xyz':
            new_line = old_line.replace(
                name + '_{}'.format(str(prev_itr).zfill(2)) + '_feb_xyz.txt',
                name + '_{}'.format(str(new_itr).zfill(2)) + '_feb_xyz.txt',
                -1
            )
        elif type == 'displacement':
            new_line = old_line.replace(
                name + '_{}'.format(str(prev_itr).zfill(2)) + '_xy_displacement.txt',
                name + '_{}'.format(str(new_itr).zfill(2)) + '_xy_displacement.txt',
                -1
            )
        elif type == 'p1':
            new_line = old_line.replace(
                name + '_{}'.format(str(prev_itr).zfill(2)) + '_p1_strain.txt',
