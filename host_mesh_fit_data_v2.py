...
def main():
    # =============================================================================#
    # fitting parameters for host mesh fitting
    ...
    #save name
    save_name = name + '_' + str(new_itr).zfill(3) + '_hmf.xyz'
    # old_file = name + '_' + str(prev_itr).zfill(3) + '_hmf.xyz'
    
    #control z array
    arr = [0,60]
    z_arr = np.vstack(((arr) * 32))
    # print(z_arr)


    # source points for fitting
    source_points_fitting_file = name + '_' + str(prev_itr).zfill(2) + '_' + str(new_itr).zfill(2) + '_v2_redo_altered_prescribed_coordinates.xyz'
    # source_points_fitting_file = old_file
    source_points_fitting = np.hstack((np.loadtxt(source_points_fitting_file, usecols=(0,1)), z_arr))#[0::4]

    # source points to be passived deformed (not fitted)
    source_points_passive_file = name + '_' + str(prev_itr).zfill(2) + '_' + str(new_itr).zfill(2) + '_v2_redo_altered_prescribed_coordinates.xyz'
    # source_points_passive_file = old_file
    source_points_passive = np.hstack((np.loadtxt(source_points_passive_file, usecols=(0,1)), z_arr))#[:128]
    # source_points_passive = np.loadtxt(source_points_passive_file, usecols=(0,1,5))[::3]

    # target point cloud (affine transform of source points for fitting)
    target_points_file = name + '_' + str(new_itr).zfill(3) + '_matlab_b_3d_0_60.xyz'
    target_points = np.loadtxt(target_points_file, usecols=(0,1,2))[::3]
    ...
    # print(source_points_fitting_hmf)
    np.savetxt(save_name, source_points_fitting_hmf, delimiter=' ', fmt='%3.6f')
    # =============================================================#
    # view
    # if has_mayavi:
    #     v = fieldvi.Fieldvi()
    #     v.addData('target points', target_points, renderArgs={'mode': 'point', 'color': (1, 0, 0)})
    #     v.addData('source points fitting', source_points_fitting, renderArgs={'mode': 'sphere'})
